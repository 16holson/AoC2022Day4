...
def findContains(line, pattern):
    ...
    if ((int(first.group(1)) <= int(second.group(1))) and int(first.group(2)) >= int(second.group(1))) or (int(second.group(1)) <= int(first.group(1))) and int(second.group(2)) >= int(first.group(1)):
        return 1
    return 0
